...
use InternalPlanlamaModel as PlanlamaModel,AyarModel,InternalCariModel as CariModel,PersonelModel;

class Etkinlik extends Controller
{
    ...
    public function main($id,$user="")
    {
        ...
        $etkinlikDetay = PlanlamaModel::etkinlikDetay($id);
        if (empty($etkinlikDetay->id)) {
            Redirect::action('s404');
            exit();
        }

        $kullanicilar = json_decode($etkinlikDetay->katilimcilar);

        if(in_array($user,$kullanicilar)){

            $etkinlikTur = PlanlamaModel::etkinlikTurDetay($etkinlikDetay->tur);

            $katilimcilar = json_decode($etkinlikDetay->katilimcilar);

            View::user($user);
            View::katilimcilar($katilimcilar);
            View::detay($etkinlikDetay);
            View::etkinlikTur($etkinlikTur);


        }else{

            Redirect::insert(['bilgi'=>'<div class="callout callout-danger">Bu etkinlikte yetkiniz yok!</div>'])->action('home');
